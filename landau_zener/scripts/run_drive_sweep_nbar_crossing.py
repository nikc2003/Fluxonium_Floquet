"""
scripts/run_drive_sweep_nbar_crossing.py
drive sweep that locates the partial transition point via
the nbar crossing between two target branches.
rather than searching for the minimum quasienergy gap within a
chi_max_gap window, this finds where the nbar of the two branches
crosses (dnbar = nbar_i - nbar_j = 0), corresponding to 50/50
hybridization. quasienergy gap at that chi == the
transition gap.
...
from landau_zener.analysis.pipeline_nbar_crossing import run_drive_sweep_nbar_crossing
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Nbar-crossing drive sweep: locates the partial transition point "
            "where nbar of two target branches crosses (50/50 hybridization)."
        )
    )
    ap.add_argument("--config", required=True, help="Path to config python file (device/config_qA.py)")
    ...
    ap.add_argument("--kappa-ghz", type=float, default=None)
    ...
    print("Nbar-crossing mode: locating transition where nbar branches cross (dnbar=0)")

    df = run_drive_sweep_nbar_crossing(
    ...
    if df is None or len(df) == 0:
        print("No events found (branches may never carry enough weight for the target pair).")
        return

    n_events = len(df)
    n_crossing = int(df["crossing_found"].sum()) if "crossing_found" in df.columns else 0
    n_g2_ok = int(df["g2_fit_ok"].sum()) if "g2_fit_ok" in df.columns else n_events
    print(f"Found {n_events} events ({n_crossing} with nbar crossing, {n_g2_ok} with valid g² fit).")

    # Report nbar crossing vs min gap comparison
    if "chi_gap_vs_cross_offset_MHz" in df.columns:
        offsets = df["chi_gap_vs_cross_offset_MHz"].dropna()
        if len(offsets) > 0:
            print(f"  chi(min gap) - chi(nbar cross) offset: "
                  f"mean={offsets.mean():.2f} MHz, max={offsets.abs().max():.2f} MHz")

    if "swap_fraction" in df.columns:
        fracs = df["swap_fraction"].dropna()
        if len(fracs) > 0:
            print(f"  Swap fraction at chi_max: mean={fracs.mean():.3f}, "
    ...
    plot_gap_vs_deltachi_scatter(
        ...
        title=f"Nbar crossing {args.bare_a}->{args.bare_b}  gap@cross vs δχ",
        filename="nbar_crossing_scatter_gap_vs_dchi.png",
        show_table=True,
    )
    ...
    dsort = df.sort_values("gap_at_cross_MHz", ascending=True).head(int(args.plot_topk))
    for k, (_, row) in enumerate(dsort.iterrows()):
        plot_event_annotated_gap(
            ...
            observable="nbar",
            outdir=args.outdir,
            filename=f"annotated_nbar_crossing_{k}.png",
            show=False,
        )
