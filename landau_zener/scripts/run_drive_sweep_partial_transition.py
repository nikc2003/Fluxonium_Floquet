"""
scripts/run_drive_sweep_partial_transition.py
does not require full swap.
"""
...
from landau_zener.analysis.pipeline_partial import run_drive_sweep_partial_transition
...
def _jsonable(x):
    """Recursively convert objects into JSON-safe builtins."""
...
def save_run_config(args, cfg, drive_freqs_GHz):
    ...
    record = {
        ...
        "resolved": {
            ...
            "chi_max_gap_MHz": None if args.chi_max_gap is None else float(args.chi_max_gap),
...
def main():
    ap = argparse.ArgumentParser(
        description="Partial-gap drive sweep: reports min gap even without full branch swap."
    )
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Maximum chi (MHz) up to which the minimum gap is searched. "
            "If omitted, the full chi sweep range (chi-min to chi-max) is used. "
            "Example: --chi-max-gap 50  searches for the min gap only up to 50 MHz chi."
        ),
    )
    ...
    chi_max_gap_MHz = args.chi_max_gap  

    if chi_max_gap_MHz is not None:
        print(f"Partial-gap mode: searching min gap up to chi = {chi_max_gap_MHz:.1f} MHz")
    else:
        print("Partial-gap mode: searching min gap over the full chi sweep range")

    df = run_drive_sweep_partial_transition(
    ...
    print(f"Found {n_events} events ({n_g2_ok} with valid g² fit, {n_events - n_g2_ok} raw min-gap only).")

    plot_gap_vs_deltachi_scatter(
        df,
        outdir=args.outdir,
        color_by="freq_GHz" if "freq_GHz" in df.columns else None,
        title=f"Partial gap {args.bare_a}->{args.bare_b}  Δmin vs δχ",
        filename="partial_scatter_Delta_vs_dchi.png",
        show_table=True,
    )
