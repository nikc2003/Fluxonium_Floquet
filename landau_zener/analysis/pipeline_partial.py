"""
landau_zener/analysis/pipeline_partial.py
bascially same thing, targeted transition sweep that reports the minimum gap between target branches
 WITHOUT requiring a full branch swap. variable chi_max_gap to limit the gap search range.
"""
...
from landau_zener.analysis.detection_partial import analyze_pair_partial_gap
...
def run_targeted_pair_partial_single_point(
    *,
    dim_q: int,
    ...
    chi_max_gap_MHz: Optional[float] = None,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    partial-gap detector for a requested bare pair (a, b).
    Like "run_targeted_pair_fullchi_single_point" but does NOT require
    the branches to complete a full swap.  Instead it searches for the
    minimum quasienergy gap between the two target-carrying branches up
    to "chi_max_gap_MHz".  but if "chi_max_gap_MHz" is None then it defaults
    full sweep range is used. This means partial transitions that would previously return
    "no target events found" now report the min gap observed within the requested chi window.
    ...
    chi_max_gap_rad = None
    if chi_max_gap_MHz is not None:
        chi_max_gap_rad = float(mhz_to_angular(chi_max_gap_MHz))

    ev = analyze_pair_partial_gap(
        sweep,
        branch_i=int(bi),
        branch_j=int(bj),
        chi_max_gap=chi_max_gap_rad,
        g2_fit_half_window=int(detect_cfg.g2_fit_half_window),
        event_prefix="TPARTIAL",
    )
    if ev is None:
        return pd.DataFrame(), dict(
...
def _drive_sweep_partial_worker(job: Dict[str, Any]) -> pd.DataFrame:
    """for run_drive_sweep_partial_transition."""
    ...
    df_b, _dbg = run_targeted_pair_partial_single_point(
        dim_q=int(job["dim_q"]),
        ...
        chi_max_gap_MHz=(
            float(job["chi_max_gap_MHz"])
            if job.get("chi_max_gap_MHz") is not None
            else None
        ),
    )
    ...
    if df_b is not None and len(df_b) > 0:
        ensure_dir(odir)
        save_df_csv(df_b, os.path.join(odir, "events_partial_gap.csv"))

    return df_b

def run_drive_sweep_partial_transition(
    *,
    ...
    """
    Drive frequency sweep with partial-gap detection.
    """
    ensure_dir(outdir)
    ...
    if parallel:
        dfs = process_pool_map(
            _drive_sweep_partial_worker,
            jobs,
            max_workers=int(max_workers),
            mp_start_method="spawn",
        )
    else:
        dfs = [_drive_sweep_partial_worker(j) for j in jobs]
    ...
    if len(out) > 0:
        save_df_csv(
            out, os.path.join(outdir, "drive_sweep_partial_gap_all.csv")
        )
